...
function hasMessageId(messages: Message[], { id }: Message): boolean {
...
export const useMessagingStore = defineStore('messaging', {
  ...
  actions: {
    deleteMessages(messages: Message[]): void {
      this.messages = this.messages.filter(message => !hasMessageId(messages, message))
    },
    toggleSelectMessage(message: Message): void {
      this.messages = this.messages.map(messageToSelect => {
        if (message.id === messageToSelect.id) {
          messageToSelect.selected = !messageToSelect.selected
        }
